...
export const notificationService = {
  ...
  getNotifications: async (token: string): Promise<PaginatedResponse<Notification>> => {
    return apiClient.get<PaginatedResponse<Notification>>("/api/notifications", { token })
  },

  // Mark notification as read
  markAsRead: async (notificationId: string, token: string): Promise<ApiResponse<Notification>> => {
    return apiClient.patch<ApiResponse<Notification>>(`/api/notifications/${notificationId}/read`, {}, { token })
  },

  // Mark all notifications as read
  markAllAsRead: async (token: string): Promise<ApiResponse<{ success: boolean }>> => {
    return apiClient.patch<ApiResponse<{ success: boolean }>>("/api/notifications/read-all", {}, { token })
  },

  // Delete notification
  deleteNotification: async (notificationId: string, token: string): Promise<ApiResponse<{ success: boolean }>> => {
    return apiClient.delete<ApiResponse<{ success: boolean }>>(`/api/notifications/${notificationId}`, { token })
  },
}
